module internal Nessos.FsPickler.PicklerFactory
    ...
    type PicklerFactory (resolver : IPicklerResolver) =
        ...
        interface ITypeShapeVisitor<Pickler> with
            ...
            member __.Abstract<'T> () =
                AbstractPickler.Create<'T> () :> Pickler

            member __.Class<'T when 'T : not struct> () =
                ClassFieldPickler.Create<'T> resolver :> Pickler

            member __.ISerializable<'T when 'T :> ISerializable> () =
                ISerializablePickler.Create<'T> resolver :> Pickler

            member __.Struct<'T when 'T : struct> () =
                StructPickler.Create<'T> resolver :> Pickler

            member __.Delegate<'T when 'T :> Delegate> () =
                DelegatePickler.Create<'T> resolver :> Pickler

            member __.Enum<'E, 'U when 'E : enum<'U>> () =
                EnumPickler.Create<'E,'U> resolver :> Pickler

            member __.Nullable<'T when   
                                'T : (new : unit -> 'T) and 
                                'T : struct and 
                                'T :> ValueType> () =

                NullablePickler.Create<'T> resolver :> Pickler
