...
FsPickler discriminates between reference types that are sealed and types that are not sealed. 
...
For this reason, picklers can only be used to serialize values whose actual type is identical
to the pickler implementation. When coming across an object of non-sealed type, FsPickler
will examine its reflected type; if found to differ, a new pickler will be generated
generated dynamically for that particular subtype.

The above implies that `obj` is serializable in FsPickler, but `Pickler<obj>`
can only be used with `obj()` instances. Similarly, picklers for interfaces
or abstract classes are possible, but they are incapable of serializing anything.
...
the following categories:
  ...
  * Pickler generators: for nominal types that are not expressions on known types,
    ...
    Pickler generators can be further separated into the following kinds:
        - Field-based/struct pickler generation.
