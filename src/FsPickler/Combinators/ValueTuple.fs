...
type internal ValueTuplePickler =

    static member Create(p1 : Pickler<'T1>) =
        let writer (w : WriteState) (_ : string) (t : ValueTuple<'T1>) =
            p1.Write w "Item1" t.Item1

        let reader (r : ReadState) (_ : string) =
            let t1 = p1.Read r "Item1"
            new ValueTuple<'T1>(t1)

        let cloner (c : CloneState) (t : ValueTuple<'T1>) =
            let t' = p1.Clone c t.Item1
            new ValueTuple<'T1>(t')

        let accepter (v : VisitState) (t : ValueTuple<'T1>) =
            p1.Accept v t.Item1
    ...
    static member Create<'T1> (resolver : IPicklerResolver) =
        ...
        ValueTuplePickler.Create p1


    static member Create(p1 : Pickler<'T1>, p2 : Pickler<'T2>) =
        let writer (w : WriteState) (_ : string) (struct (t1,t2) : struct ('T1 * 'T2)) =
            p1.Write w "Item1" t1
            p2.Write w "Item2" t2
        ...
        let cloner (c : CloneState) struct (t1,t2) =
            let t1' = p1.Clone c t1
            let t2' = p2.Clone c t2
            struct (t1',t2')

        let accepter (v : VisitState) struct (t1,t2) =
            p1.Accept v t1
            p2.Accept v t2
    ...
    static member Create<'T1, 'T2> (resolver : IPicklerResolver) =
        ...
        ValueTuplePickler.Create(p1, p2)

    static member Create(p1 : Pickler<'T1>, p2 : Pickler<'T2>, p3 : Pickler<'T3>) =
        let writer (w : WriteState) (_ : string) (struct (t1,t2,t3) : struct ('T1 * 'T2 * 'T3)) =
            p1.Write w "Item1" t1
            p2.Write w "Item2" t2
            p3.Write w "Item3" t3
        ...
        let cloner (c : CloneState) struct (t1,t2,t3) =
            let t1' = p1.Clone c t1
            let t2' = p2.Clone c t2
            let t3' = p3.Clone c t3
            struct (t1',t2',t3')

        let accepter (v : VisitState) struct (t1,t2,t3) =
            p1.Accept v t1
            p2.Accept v t2
            p3.Accept v t3
    ...
    static member Create<'T1, 'T2, 'T3> (resolver : IPicklerResolver) =
        ...
        ValueTuplePickler.Create(p1, p2, p3)


    static member Create(p1 : Pickler<'T1>, p2 : Pickler<'T2>, p3 : Pickler<'T3>, p4 : Pickler<'T4>) =
        let writer (w : WriteState) (_ : string) (struct (t1,t2,t3,t4) : struct ('T1 * 'T2 * 'T3 * 'T4)) =
            p1.Write w "Item1" t1
            p2.Write w "Item2" t2
            p3.Write w "Item3" t3
            p4.Write w "Item4" t4
        ...
        let cloner (c : CloneState) struct (t1,t2,t3,t4) =
            let t1' = p1.Clone c t1
            let t2' = p2.Clone c t2
            let t3' = p3.Clone c t3
        ...
        let accepter (v : VisitState) struct (t1,t2,t3,t4) =
            p1.Accept v t1
            p2.Accept v t2
            p3.Accept v t3
            p4.Accept v t4
    ...
    static member Create<'T1, 'T2, 'T3, 'T4> (resolver : IPicklerResolver) =
        ...
        ValueTuplePickler.Create(p1, p2, p3, p4)


    static member Create(p1 : Pickler<'T1>, p2 : Pickler<'T2>, p3 : Pickler<'T3>, p4 : Pickler<'T4>, p5 : Pickler<'T5>) =
        let writer (w : WriteState) (_ : string) (struct (t1,t2,t3,t4,t5) : struct ('T1 * 'T2 * 'T3 * 'T4 * 'T5)) =
            p1.Write w "Item1" t1
            p2.Write w "Item2" t2
            p3.Write w "Item3" t3
            p4.Write w "Item4" t4
        ...
        let cloner (c : CloneState) struct (t1,t2,t3,t4,t5) =
            let t1' = p1.Clone c t1
            let t2' = p2.Clone c t2
            let t3' = p3.Clone c t3
            let t4' = p4.Clone c t4
        ...
        let accepter (v : VisitState) struct (t1,t2,t3,t4,t5) =
            p1.Accept v t1
            p2.Accept v t2
            p3.Accept v t3
            p4.Accept v t4
    ...
    static member Create<'T1, 'T2, 'T3, 'T4, 'T5> (resolver : IPicklerResolver) =
        ...
        ValueTuplePickler.Create(p1, p2, p3, p4, p5)


    static member Create(p1 : Pickler<'T1>, p2 : Pickler<'T2>, p3 : Pickler<'T3>, p4 : Pickler<'T4>, p5 : Pickler<'T5>, p6 : Pickler<'T6>) =
        let writer (w : WriteState) (_ : string) (struct (t1,t2,t3,t4,t5,t6) : struct ('T1 * 'T2 * 'T3 * 'T4 * 'T5 * 'T6)) =
            p1.Write w "Item1" t1
            p2.Write w "Item2" t2
            p3.Write w "Item3" t3
            p4.Write w "Item4" t4
        ...
        let cloner (c : CloneState) struct (t1,t2,t3,t4,t5,t6) =
            let t1' = p1.Clone c t1
            let t2' = p2.Clone c t2
            let t3' = p3.Clone c t3
            let t4' = p4.Clone c t4
        ...
        let accepter (v : VisitState) struct (t1,t2,t3,t4,t5,t6) =
            p1.Accept v t1
            p2.Accept v t2
            p3.Accept v t3
            p4.Accept v t4
    ...
    static member Create<'T1, 'T2, 'T3, 'T4, 'T5, 'T6> (resolver : IPicklerResolver) =
        ...
        ValueTuplePickler.Create(p1, p2, p3, p4, p5, p6)
    ...
    static member Create(p1 : Pickler<'T1>, p2 : Pickler<'T2>, p3 : Pickler<'T3>, 
        ...
        let writer (w : WriteState) (_ : string) (struct (t1,t2,t3,t4,t5,t6,t7) : struct ('T1 * 'T2 * 'T3 * 'T4 * 'T5 * 'T6 * 'T7)) =
            p1.Write w "Item1" t1
            p2.Write w "Item2" t2
            p3.Write w "Item3" t3
            p4.Write w "Item4" t4
        ...
        let cloner (c : CloneState) struct (t1,t2,t3,t4,t5,t6,t7) =
            let t1' = p1.Clone c t1
            let t2' = p2.Clone c t2
            let t3' = p3.Clone c t3
            let t4' = p4.Clone c t4
        ...
        let accepter (v : VisitState) struct (t1,t2,t3,t4,t5,t6,t7) =
            p1.Accept v t1
            p2.Accept v t2
            p3.Accept v t3
            p4.Accept v t4
    ...
    static member Create<'T1, 'T2, 'T3, 'T4, 'T5, 'T6, 'T7> (resolver : IPicklerResolver) =
        ...
        ValueTuplePickler.Create(p1, p2, p3, p4, p5, p6, p7)


    static member Create(p1 : Pickler<'T1>, p2 : Pickler<'T2>, p3 : Pickler<'T3>, p4 : Pickler<'T4>,
                            p5 : Pickler<'T5>, p6 : Pickler<'T6>, p7 : Pickler<'T7>, pr : Pickler<'TRest>) =

        let writer (w : WriteState) (_ : string) (tuple : ValueTuple<'T1,'T2,'T3,'T4,'T5,'T6,'T7,'TRest>) =
        ...
        let reader (r : ReadState) (_ : string) =
            ...
            new ValueTuple<_,_,_,_,_,_,_,_>(t1,t2,t3,t4,t5,t6,t7,rest)

        let cloner (c : CloneState) (tuple : ValueTuple<'T1,'T2,'T3,'T4,'T5,'T6,'T7,'TRest>) =
        ...
        let accepter (v : VisitState) (tuple : ValueTuple<'T1,'T2,'T3,'T4,'T5,'T6,'T7,'TRest>) =
    ...
    static member Create<'T1, 'T2, 'T3, 'T4, 'T5, 'T6, 'T7, 'TRest when 'TRest : (new : unit -> 'TRest) and 'TRest : struct and 'TRest :> ValueType> (resolver : IPicklerResolver) =
        let p1 = resolver.Resolve<'T1> ()
        let p2 = resolver.Resolve<'T2> ()
        let p3 = resolver.Resolve<'T3> ()
        let p4 = resolver.Resolve<'T4> ()
        ...
        ValueTuplePickler.Create(p1, p2, p3, p4, p5, p6, p7, pr)